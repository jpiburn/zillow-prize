...
read in and join together our information on properties
```{r read-prop-filtered, warning=FALSE, message=FALSE, eval=FALSE}
library(feather)
library(tidyverse)
...
The number one rule in real estate is of course

> "Location, Location, Location."
>
> --- Some Real Estate Guy

Based on this idea, let's create some new features
...
Because these features will have values for every day from `min(transactions$date)` to `max(transactions$date)` we won't join them to our data yet.
...
trans_prop <- properties_geo %>%
  right_join(transactions, by = "id_parcel") %>%
  select(
    id_parcel,
    id_geo_bg_fips,
    id_geo_tract_fips,
...
The American Community Survey is a great source of demographic and household data. As an example of using that data let's bring in a few features related to our area of interest. 
...
properties <- properties %>%
  mutate(
    ...
    area_lot_quantile = cut(area_lot_jitter, breaks = quantile(area_lot_jitter, probs = seq(0, 1, 0.1), na.rm = TRUE))
    ) %>%
  group_by(
    id_geo_bg_fips,
    zoning_landuse,
...
At this point we now have all the features we are going to use and have filled in all missing values. The next step is to transform our features and then do a basic level of feature selection. 
